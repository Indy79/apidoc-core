...
function parse(options) {
    ...
    try {
        ...
        app.log.verbose('run parser for common apiDefine');

        if (options.common instanceof Array) {
            options.common.forEach(function(folder) {
                // Keep same options for each folder, but ensure the 'src' of options
                // is the folder currently being processed.
                var folderOptions = _.cloneDeep(options);
